...
import { createNewLoan, LoanType } from "./AddSubType";
import { LoanProvider, TypeProvider } from "./TypeProvider";
import { createNewAccessory } from "./AddAsset";
import { createNewType, useAddAssets, useLoans } from "./AddAssetsProvider";
...
export const AddAssetStep1 = () => {
    ...
    const validate = values => {
      ...
      values.types.forEach((type, typeIndex) => {
        ...
        const stIdDuplicates = validateUniqueValues(type.subTypes, ['typeId']);
        ...
        >
          {({ values, errors }) => {
            return (
              <Form>
                <ModalBody>
                  <ExcelFormControl loadValues={setValuesExcel} templateCols={['type', 'subType', 'assetTag', 'serialNumber', 'remarks']}/>
                  <Divider borderColor="black" borderWidth="2px" my={2} />
                  <FieldArray name="types">
                  {typeHelpers => (
                    values.loans.map((type, typeIndex, array) => (
