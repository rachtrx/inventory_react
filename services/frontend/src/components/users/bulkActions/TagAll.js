...
export const TagAll = () => {
    ...
    const { tagOptions, setTagOptions, addNewTag } = useUserTags();
    const [ validTagOptions, setValidTagOptions ] = useState([]);
    const [ key, setKey ] = useState(1)

    useEffect(() => {
        console.log(tagOptions);
        const allTagIds = new Set(users.flatMap(u => u.tags?.map(t => t.tagId) || []));
    ...
    if (!users?.length) return <BulkActionButton onClick={() => setFormType(FormType.TAG_USER)}>Add Tags</BulkActionButton>;
                ...
                >
                    <Form>
                        <Flex direction="column" gap={2}>
                            <Text fontSize="lg">Tag Users</Text>
                            <VStack
                                align="center"
                                overflowY="auto"
                                maxH="300px"
